import { Link } from "react-router-dom";

export default function Navbar() {
  return (
    <nav className="bg-blue-700 text-white p-4 flex justify-between items-center shadow-lg">
      <h1 className="text-2xl font-bold">Lengua de Señas Colombiana</h1>
      <ul className="flex gap-4">
        <li><Link to="/" className="hover:text-yellow-300">Inicio</Link></li>
        <li><Link to="/login" className="hover:text-yellow-300">Login</Link></li>
        <li><Link to="/register" className="hover:text-yellow-300">Registro</Link></li>
        <li><Link to="/courses" className="hover:text-yellow-300">Cursos</Link></li>
      </ul>
    </nav>
  );
}
